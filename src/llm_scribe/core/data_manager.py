...
from llm_scribe.core.security import sanitize_input, safe_path_check
...
class DataManager:
    ...
    def save_config(self, new_data_file=None):
        ...
        with self.lock:
            if new_data_file: self.data_file = new_data_file
            try:
        ...
        with self.lock:
            for s in self.data["sessions"]:
                if s["id"] == sid:
                    ...
                    if title is not None: s["title"] = sanitize_input(title)
                    if tags is not None: s["tags"] = tags
                    if parent is not None: s["parent"] = parent
                    break
        self.save_data()
